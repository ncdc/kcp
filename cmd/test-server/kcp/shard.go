...
func (s *Shard) WaitForReady(ctx context.Context) (<-chan error, error) {
	// wait for readiness
	klog.Infof("Waiting for %s Shard /readyz to succeed", s.name)
	for {
		...
		if err := res.Error(); err != nil {
			klog.V(3).Infof("kcp Shard %s not ready: %v", s.name, err)
		} else {
			var rc int
			res.StatusCode(&rc)
			if rc == http.StatusOK {
			...
			if bs, err := res.Raw(); err != nil {
				...
				klog.V(3).Infof("kcp Shard %s not ready: http %d: %s", s.name, rc, string(bs))
			}
		}
	}
	if !klog.V(3).Enabled() {
